...
/// Per-slot values that used to ride mesh3d/sky push constants. They change
/// every frame (jittered view-proj, camera eye, sun disc) while the recorded
/// command stream stays the same, so they live in the frame UBO (std140).
/// Not part of the public `FrameUniformsGpu` wire.
#[derive(Clone, Copy)]
enum RecTy {
    /// Column-major float4x4: 16-byte aligned, 64 bytes.
    Mat4,
    F32,
    F32x2,
    I32x3,
...
impl RecTy {
    fn align(self) -> usize {
        match self {
            RecTy::Mat4 | RecTy::I32x3 | RecTy::F32x3 | RecTy::F32x4 => 16,
            RecTy::F32x2 => 8,
            RecTy::F32 | RecTy::I32 => 4,
        }
    }
...
fn record_field_table() -> Vec<RecField> {
    vec![
        RecField {
            name: "view_proj",
            ty: RecTy::Mat4,
            doc: "Jittered camera clip matrix (was mesh3d vertex push). Column-major.",
        },
        RecField {
            name: "clip",
            ty: RecTy::F32,
            doc: "LOD slab xz radius; 0 disables the fragment discard (was mesh3d push).",
        },
        RecField {
            name: "clip_v",
            ty: RecTy::F32,
            doc: "LOD slab vertical half-height (was mesh3d push).",
        },
        RecField {
            name: "clip_pad",
            ty: RecTy::F32x2,
            doc: "std140 pad after clip/clip_v so cam_block is 16-byte aligned.",
        },
        RecField {
            name: "cam_block",
            ty: RecTy::I32x3,
            doc: "Camera eye integer block (was mesh3d vertex push).",
        },
        RecField {
            name: "cam_pad0",
            ty: RecTy::I32,
            doc: "std140 pad after cam_block.",
        },
        RecField {
            name: "cam_frac",
            ty: RecTy::F32x3,
            doc: "Camera eye fractional part in [0,1) (was mesh3d vertex push).",
        },
        RecField {
            name: "cam_pad1",
            ty: RecTy::F32,
            doc: "std140 pad after cam_frac.",
        },
        RecField {
            name: "inv_view_proj",
            ty: RecTy::Mat4,
            doc: "Inverse jittered clip matrix for sky view-ray reconstruction (was sky push).",
        },
        RecField {
            name: "sun",
            ty: RecTy::F32x4,
            doc: "xyz unit sun dir, w = cos(radius * SUN_DISC_CORE) (was sky push).",
        },
        RecField {
            name: "sun_tint",
            ty: RecTy::F32x4,
            doc: "rgb linear disc tint, w = cos(radius * SUN_DISC_RIM) (was sky push).",
        },
        RecField {
            name: "moon",
            ty: RecTy::F32x4,
            doc: "x = moon cos_core, y = moon cos_rim (was sky push).",
        },
    ]
}
...
fn emit_rust_uniforms(public: &[Lane], derived: &[Lane], record: &[RecField]) -> String {
    let mut s = String::new();
    emit_generated_spdx(&mut s);
    s.push_str("// @generated by voxel-engine/build.rs — DO NOT EDIT.\n");
    s.push_str("// Source of truth: lane_table() / derived_lane_table() / record_field_table()\n");
    s.push_str("// in build.rs.\n");
    s.push_str("// Twin file: shaders/generated/frame_uniforms.slang (public then derived then record).\n\n");
    ...
    s.push('\n');
    s.push_str("/// GPU UBO: public [`FrameUniformsGpu`] plus the engine-derived tail and the\n");
    s.push_str("/// per-slot record-varying matrices/eye/sky disc (std140).\n");
    ...
    for f in record {
        doc_lines(f.doc, "    /// ", &mut s);
        s.push_str(&format!("    pub {}: {},\n", f.name, f.ty.rust()));
    }
    s.push_str("}\n\n");
    let mut off = (public.len() + derived.len()) * 16;
    let record_offsets: Vec<usize> = record
        .iter()
        .map(|f| {
            off = align_up(off, f.ty.align());
            let at = off;
            off += f.ty.size();
            at
        })
    ...
    for (f, at) in record.iter().zip(record_offsets) {
        s.push_str(&format!(
            "const _: () = assert!(std::mem::offset_of!(FrameUniformsExt, {}) == {});\n",
            f.name, at
        ));
...
fn emit_slang_uniforms(public: &[Lane], derived: &[Lane], record: &[RecField]) -> String {
    let mut s = String::new();
    emit_generated_spdx(&mut s);
    s.push_str("// @generated by voxel-engine/build.rs — DO NOT EDIT.\n");
    s.push_str("// Source of truth: lane_table() / derived_lane_table() / record_field_table()\n");
    ...
    s.push_str("// the engine-derived tail and record-varying fields on FrameUniformsExt.\n");
    s.push_str(
    ...
    if !record.is_empty() {
        s.push_str("    // --- per-slot record-varying (was mesh3d / sky push constants) ---\n");
        for f in record {
            doc_lines(f.doc, "    // ", &mut s);
            s.push_str(&format!("    {} {};\n", f.ty.slang(), f.name));
        }
...
fn generate_shared_constants(out_dir: &Path) {
    ...
    let record = record_field_table();
    write_if_changed(
        &out_dir.join("gen_frame_uniforms.rs"),
        &emit_rust_uniforms(&public, &derived, &record),
    );
    write_if_changed(
        &gen_dir.join("frame_uniforms.slang"),
        &emit_slang_uniforms(&public, &derived, &record),
    );
}
