...
fn main() {
    ...
    // Opaque-only mesh3d.frag variants. `MESH3D_OPAQUE` strips the water/absorb
    // branch and the LOD-slab `discard`, so the full-res opaque module carries
    // no OpKill (early depth write stays enabled) and no dead water code.
    // `MESH3D_LOD` re-adds the slab clip (`discard`) for the coarse-LOD
    // partition, which never samples the cascades at all.
    let mesh3d_opaque = Shader {
        src: "shaders/mesh3d.frag.slang",
        stage: "fragment",
        entry: "fragmentMain",
    ...
    if refresh_fallbacks {
        let variants = [&mesh3d_water, &mesh3d_opaque, &mesh3d_lod];
        for shader in SHADERS.iter().chain(variants.into_iter()) {
...
fn lane_table() -> Vec<Lane> {
    vec![
        ...
        Lane {
            name: "extras",
            doc: "x = stars gain (1 = night starfield renders, 0 = skipped — the\n`RenderFlags::stars` gate). yzw reserved on the public wire (always zero);\nrepurposing a channel bumps FRAME_UNIFORMS_VERSION. The renderer may overwrite\nthe whole lane for debug-flat (TerrainKey).",
        },
        Lane {
            name: "anim",
